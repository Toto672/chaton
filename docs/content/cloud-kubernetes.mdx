...
- `cloud-api` now supports a PostgreSQL-backed control plane for users, cloud workspaces, projects, conversations, transcripts, durable plan definitions, and stateful desktop auth requests, while still keeping an in-memory fallback for local development when `DATABASE_URL` is absent.
...
- Replace the current demo desktop auth flow with a real OIDC/PKCE exchange backed by the configured issuer
