...
```
^@[^/]+\/chatons-[a-z0-9][a-z0-9-]*$
```

**Valid examples:**

- `@john/chatons-my-notes`
- `@acme/chatons-extension-weather`
- `@thibaut/chatons-channel-telegram`

**Invalid examples:**

- `my-notes` (missing `@scope/chatons-` prefix)
- `@john/notes` (missing `chatons-` prefix)
- `@john/chatons-` (no name after prefix)
...
**Important:** The `name` in `package.json` and the `id` in `chaton.extension.json` must follow the same `@user/chatons-*` pattern. They should match.
...
| Error | Solution |
|-------|----------|
| `Invalid package name` | Name must match `@user/chatons-*` pattern |
