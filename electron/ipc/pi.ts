...
import { generateMetaHarnessHumanReport } from '../meta-harness/explainer.js';
...
function resolveMetaHarnessAttemptResult(
  agentDir: string,
  input: {
    runId?: string | null
  ...
  return {
    ...
    attempt,
    selectedCandidateId,
    candidate: readJsonFileIfExists(path.join(candidateRoot, 'candidate.json')) as Record<string, unknown> | null,
    score: readJsonFileIfExists(path.join(candidateRoot, 'score.json')) as Record<string, unknown> | null,
    summary: readJsonFileIfExists(path.join(candidateRoot, 'summary.json')) as Record<string, unknown> | null,
...
export function registerPiIpc() {
  ...
  ipcMain.handle('meta-harness:getOptimizerAttemptResult', (_event, input: {
    ...
    return resolveMetaHarnessAttemptResult(agentDir, input)
  })

  ipcMain.handle('meta-harness:generateHumanReport', async (_event, input: {
    runId?: string | null
    benchmarkId?: string | null
    attemptId?: string | null
    candidateId?: string | null
  }) => {
    const resolved = resolveMetaHarnessAttemptResult(agentDir, input)
    return generateMetaHarnessHumanReport(resolved)
  })
