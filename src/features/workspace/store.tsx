...
function reducer(state: WorkspaceState, action: Action): WorkspaceState {
  switch (action.type) {
    case "hydrate": {
      ...
      const firstConversation = action.payload.conversations.find(
      ...
      return {
        ...
        selectedConversationId:
          firstConversation?.id ?? action.payload.conversations[0]?.id ?? null,
      };
    }
    case "selectProject": {
      const firstConversation = state.conversations.find(
        (conversation) => conversation.projectId === action.payload.projectId,
      );
      return {
        ...state,
        selectedProjectId: action.payload.projectId,
        selectedConversationId:
          firstConversation?.id ?? state.selectedConversationId,
      };
    }
    case "selectConversation": {
      ...
      return {
        ...
        selectedProjectId: conversation
    ...
    case "toggleProjectCollapsed": {
      const exists = state.settings.collapsedProjectIds.includes(
        action.payload.projectId,
      );
      const collapsedProjectIds = exists
        ? state.settings.collapsedProjectIds.filter(
    ...
    case "addProject": {
      return {
        ...
        settings: {
          ...state.settings,
          collapsedProjectIds: state.settings.collapsedProjectIds.filter(
            (id) => id !== action.payload.project.id,
          ),
        },
      };
    }
...
export function WorkspaceProvider({ children }: PropsWithChildren) {
  ...
  const value = useMemo(
    () => ({
      ...
      startConversationDraft: (projectId: string) =>
        dispatch({ type: "startConversationDraft", payload: { projectId } }),
      ...
      setNotice: (notice: string | null) =>
        dispatch({ type: "setNotice", payload: { notice } }),
    }),
    [
      createConversationForProject,
      importProject,
